...
def build_skinport_sale(*, item_name='Sticker | Lorena (Holo)', sale_price=125, wear=None):
    return {
...
class TestConvertIsoStrToSeattleStr(ParserUnitTestCase):
    def test_converts_utc_timestamp_to_purchase_timezone_date(self):
        converted = parser.convert_iso_str_to_seattle_str('2025-03-24T09:36:35.811079+00:00')
...
class TestParseCSFloatData(ParserUnitTestCase):
    def test_aggregates_fixture_data_and_coerces_commodity_float_to_none(self):
        ...
        parser.parse_csfloat_data(aggregated_data, [str(TEST_DIR / 'csfloat' / 'test_page0_trades.json')])
    ...
    def test_raises_when_trade_shape_does_not_match_expected_contract_fields(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            file_path = self.write_json(
                ...
                {
                    'count': 1,
                    'trades': [{'state': 'verified', 'accepted_at': '2025-03-24T09:36:35.811079+00:00'}],
                },
            )
...
class TestParseSCMData(ParserUnitTestCase):
    def test_parses_fixture_rows_into_expected_keys(self):
        ...
        parser.parse_scm_data(aggregated_data, [str(TEST_DIR / 'scm' / 'scm_purchase_25.csv')])
    ...
    def test_rewrites_2024_acted_on_dates_to_2025(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            file_path = self.write_csv_fixture(
                ...
                'scm.csv',
                [
                    ['Index', 'Credit', 'Transaction ID', 'App ID', 'Name', 'Price', 'Listed On', 'Acted On', 'Amount'],
                    [1, 0, '1-2', 730, 'Recoil Case', '$0.40', '2024-15-03', '2024-15-03', 2],
                ],
            )
...
class TestParseSkinportData(ParserUnitTestCase):
    def test_aggregates_fixture_sales_by_name_date_and_float(self):
        ...
        parser.parse_skinport_data(aggregated_data, [str(TEST_DIR / 'skinport' / 'skinport_trades.json')])
                        ...
                        'orders': [
                            {
                                ...
                                'sales': [
                                    build_skinport_sale(item_name='Sticker | Lorena (Holo)', sale_price=125),
                                    build_skinport_sale(item_name='Sticker | Lorena (Holo)', sale_price=130),
                                ],
                            }
                        ]
                    }
...
class TestWriteCsv(ParserUnitTestCase):
    def test_writes_sorted_rows_and_serializes_none_float_values(self):
        aggregated_data = self.make_aggregated_data()
        aggregated_data[('B Item', '2025-01-02', 0.5)] = parser.CSV_Tail(csf_qty=1, csf_price=100)
        aggregated_data[('A Item', '2025-01-01', None)] = parser.CSV_Tail(scm_qty=2, scm_price=250)
...
class TestWriteSummaryCsv(ParserUnitTestCase):
    def test_groups_rows_by_item_name(self):
        aggregated_data = self.make_aggregated_data()
        aggregated_data[('Item A', '2025-01-01', None)] = parser.CSV_Tail(csf_qty=1, csf_price=100)
        aggregated_data[('Item A', '2025-01-02', 0.4)] = parser.CSV_Tail(scm_qty=2, scm_price=250, skinport_qty=1, skinport_price=50)
        ...
        self.assertEqual(rows[0][0:4], ['Name', 'Total Qty', 'Subtotal', 'Pre-fee cost basis'])
...
class TestWriteCasemoveCsv(ParserUnitTestCase):
    def test_writes_cost_basis_rows_for_casemove_import(self):
        ...
        self.assertEqual(rows[0], ['Name', 'Date', 'Quantity', 'Price', 'Type', 'Note', 'Currency'])
        self.assertEqual(rows[1][0:4], ['Item A', '2025-01-01', '1', '1.1322999999999999'])
...
class TestRunner(unittest.TestCase):
    def run_snapshot(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            expected_stdout = (Path(tmpdir) / 'test' / 'expected_stdout.txt').read_text(encoding='utf-8')
            expected_csv = (Path(tmpdir) / 'test' / 'expected_output.csv').read_text(encoding='utf-8')
    ...
    def test_stdout_matches_snapshot(self):
        ...
        self.maxDiff = None
        self.assertEqual(stdout, expected_stdout, msg='runner stdout did not match expected_stdout.txt')

    def test_csv_matches_snapshot(self):
        _, output_csv, _, expected_csv = self.run_snapshot()

        self.maxDiff = None
        self.assertEqual(output_csv, expected_csv, msg='output.csv did not match expected_output.csv')
